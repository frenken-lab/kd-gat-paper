...
## Resource impact of `myst start`

`myst start` is acceptable on a login node — it idles on inotify file watches and only
spikes briefly on `.md` saves.
...
Example: paper site (`myst start`, port 3000) + colloquium slides (static server, port 3001).
...
│          └── now an OSC shell → run: cd ~/kd-gat-paper && make dev-myst
└── Tab 2  ssh rf15@pitzer-login01.hpc.osc.edu
           └── OSC shell → run slides watcher + static server (see below)

Browser (local): localhost:3000 = paper   localhost:8080/candidacy.html = slides
```

Tab 1 does double duty: the `-L` flags establish both port forwards while the shell
itself runs `make dev-myst`. Tab 2 is a plain second SSH session for the slides process.
...
> **General rule:** one blocking process per terminal tab. Each `myst start`,
...
Once on a compute node, `myst start` won't be reachable via the standard login-node
