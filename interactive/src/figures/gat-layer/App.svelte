<script>
  ...
  const h1 = buildGraph({
    n, topology: 'sparse', color: 'gat', prefix: 'h1', labels: 'auto', scale: 50,
    container: { label: 'Head 1', color: 'gat' },
  });
  const h2 = buildGraph({
    n, topology: 'sparse', color: 'gat', prefix: 'h2', labels: 'auto', scale: 50,
    container: { label: 'Head 2', color: 'gat' },
  });
  const h3 = buildGraph({
    n, topology: 'sparse', color: 'gat', prefix: 'h3', labels: 'auto', scale: 50,
  ...
  const SUBSCRIPTS = '₁₂₃₄₅₆₇₈₉';
  const headAlphas = {
    h1: [[0,1,{weight:0.23}],[0,2,{weight:0.41}],[1,2,{weight:0.18}],[2,3,{weight:0.55}],[3,4,{weight:0.32}],[4,0,{weight:0.31}]],
    h2: [[0,1,{weight:0.15}],[0,2,{weight:0.50}],[1,2,{weight:0.33}],[2,3,{weight:0.28}],[3,4,{weight:0.47}],[4,0,{weight:0.27}]],
    h3: [[0,1,{weight:0.38}],[0,2,{weight:0.12}],[1,2,{weight:0.45}],[2,3,{weight:0.19}],[3,4,{weight:0.60}],[4,0,{weight:0.26}]],
  };
  ...
  for (const head of ['h1', 'h2', 'h3']) {
    ...
    const attn = buildGraph({
      ...
      directed: true,
      edges: headAlphas[head].map(([i, j, a]) => [i, j, { ...a, type: 'encoded', color: 'attention' }]),
    ...
    const dFlow = connectContainers(unpacked.containers, { labels: ['W, a', 'σ(Σα·Wh)'] });
...
<Figure title="GAT Attention Layer">
  ...
  <Plot width={700} height={280} grid={false} axes={false} frame={false}
    ...
    <!-- Outer GAT layer box -->
    <Rect data={[outer]} x1="x1" y1="y1" x2="x2" y2="y2"
      fill="none" stroke="#999" strokeWidth={1} strokeDasharray="4 3" rx={8} />
    <!-- Head containers (clickable) -->
    <Rect data={data.containers} x1="x1" y1="y1" x2="x2" y2="y2"
  ...
  {#if detailData}
    <div style="margin-top: 8px; border-top: 1px solid #ddd; padding-top: 8px;">
      <div class="controls">
        <span style="font-size: 12px; font-weight: 600; color: #333;">{headLabel} — Internal Structure</span>
        <button class="toggle active" on:click={() => { selectedHead = null; }}>Close</button>
      </div>
      <Plot width={700} height={260} grid={false} axes={false} frame={false}
        x={{ domain: detailData.xDomain }} y={{ domain: detailData.yDomain }} inset={10}>
        <!-- Stage containers -->
        <Rect data={detailData.containers} x1="x1" y1="y1" x2="x2" y2="y2"
          fill="fill" stroke="stroke" strokeWidth={1} rx={6} />
        <Text data={detailData.containers}
          x={c => (c.x1 + c.x2) / 2} y="y1"
          text="label" fontSize={9} fill="#333" textAnchor="middle" dy={-8} />
        <!-- Flow arrows between stages -->
        <Arrow data={detailData.flowArrows} x1="x1" y1="y1" x2="x2" y2="y2"
          stroke="stroke" strokeWidth={1.5} />
        <Text data={detailData.flowArrows}
        ...
        <Link data={detailData.edges.structural} x1="x1" y1="y1" x2="x2" y2="y2"
          stroke="stroke" strokeOpacity={0.4} strokeWidth={1.5} />
        <!-- Encoded edges (attention — weight drives width + opacity) -->
        <Arrow data={detailData.edges.encoded} x1="x1" y1="y1" x2="x2" y2="y2"
          stroke="stroke"
          strokeWidth={e => 0.5 + (e.weight || 0) * 4}
          strokeOpacity={e => 0.3 + (e.weight || 0) * 0.7} />
        <!-- Nodes -->
        <Dot data={detailData.nodes} x="x" y="y" r={14}
          fill="fill" stroke="stroke" strokeWidth={1.5} />
        <Text data={detailData.nodes} x="x" y="y" text="label"
          fontSize={8} fill="#333" textAnchor="middle" dy={1} />
      </Plot>
    </div>
  {/if}
