...
4. **Stability bounds.** Lipschitz constants on GAT attention against graph-structural perturbations (edge addition/removal) drawn from the [](#subsec:Adversarial) perturbation set; yields stability bounds *and* adversarial-robustness data simultaneously.
...
3. **DP-SGD × curriculum interaction.** Re-derive the privacy accounting under amplification-by-sampling for the time-varying $p_t$ of [](#subsec:Curriculum) — a theoretical contribution at the [](#subsec:FL) ∩ [](#subsec:Curriculum) intersection, the only FL theory contribution within reach without fleet-data access.
...
Post-hoc empirical work on existing checkpoints unlocks four chapters' worth of claims without retraining. Recommended sequencing: (1) class-conditional ECE and risk-coverage curves on the calibration split ([](#subsec:Calibration)), (2) regime-stratified F1 by slip-angle bucket ([](#subsec:PINN)), (3) @adebayo2018sanity sanity-check + deletion-AUC pass on GAT attention ([](#subsec:XAI)), (4) parameter-distance test for curriculum convergence ([](#subsec:Curriculum)). The retraining set follows: capacity-ratio sweep, $N \in \{2,3,4\}$ scaling, FL baseline sweep, curriculum-momentum sensitivity.
