...
### What "reward shift" means in this framework, formally

The training reward Eq. {eq}`eq-reward` (`paper/content/methodology.md:130`–`:140`) is a *proxy*: it depends on ground-truth labels $y_{\text{true}}$ that are unavailable at deployment. Writing the deployed reward as $R_{\text{deploy}}$ and the training reward as $R_{\text{train}}$, the shift can be decomposed into two distinct phenomena:
...
### How this framework specifically supports each strategy
...
| 15-dim fusion state (`paper/content/methodology.md:126`) | Includes VGAE+GAT confidence | Reward-proxy drift detector candidate; an EKF-innovation feature from Q1.2 would extend this to estimator-pipeline drift |
| Backbone retraining cycle (`paper/content/methodology.md:198`) | Periodic representation update every 50 episodes | The natural insertion point for CQL/BCQ-style conservative updates (strategy 3) |
...
The point of the table: most of strategy (2) is already there, strategy (4)'s mechanism is already specified in Q1.1 but not wired into the fusion policy, and strategies (1), (3), (5) all become straightforward once the simplex formulation from Q4.2 lands.
...
- **Q-learning** must estimate $Q(s, a)$ on every reachable $(s, a)$ pair. The DQN [@mnih2013playingatarideepreinforcement; @mnih2015human] amortises this through a function approximator, so the explicit Q-table does not blow up — but the *exploration* problem does. With $\epsilon$-greedy at $\epsilon_0 = 0.2$ (`paper/content/methodology.md:165`) the expected number of episodes to visit each action once is $K^N / \epsilon_0 \approx 10^6$ at $N{=}4$, infeasible at the per-graph episode budget of CAN training.
...
The DRL-IDS survey [@drlids_survey2024] flags discretisation-induced action blow-up as the dominant failure mode of RL-based intrusion detectors as architectural complexity grows; the candidate approaches below all attack this directly.
...
| **Hierarchical RL** | Outer $2^N$ subset, inner $K^{|\text{subset}|}$ | High; subset-conditional | $\tilde{O}(2^N + \mathbb{E}_{|\text{subset}|}[K^{|\text{subset}|/2}\sqrt{T}])$ | Subset-level coordination | Naturally handles graceful degradation (a degraded expert is a subset action); interpretable | Two-level credit assignment; outer subset exploration adds episode overhead |
...
|---|---|---|---|
| State dim | 15 (`paper/content/methodology.md:126`) | $\approx 25$ | $f_\theta : \mathbb{R}^{d_s} \to \mathbb{R}^d$ shared backbone unchanged |
...
There is empirical support for the continuous-action choice already in this framework. The DQN at $N=2$ shows multimodal weight distributions with peaks at $\alpha \in \{0, 0.2, 0.4, 0.6, 0.8\}$ (`paper/content/explainability.md:42`, [](#fig-fusion)) — i.e., the policy has converged to roughly five discrete operating modes rather than exploiting the full 21-bin resolution. This means: (i) the current discretisation is likely over-resolved even at $N=2$, and (ii) the modes are interpretable as attack-type-specific strategies, exactly the structure a continuous policy with KL-regularised exploration would discover automatically without enumerating the grid.
...
- **Graceful-degradation protocol.** The continuous formulations *should* degrade smoothly under inference-time expert dropout (mask one expert; observe that the policy reweights without retraining). Factored bandits should degrade more abruptly because per-arm parameters are independent. This is a single experiment that distinguishes the candidates and is not currently in the proposed protocol of `paper/candidacy/proposed-research.md` §Adversarial Robustness.
