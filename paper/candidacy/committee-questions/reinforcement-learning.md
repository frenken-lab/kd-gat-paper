...
The training reward is a proxy. It depends on ground-truth labels that don't exist at deployment — at runtime, the system has to estimate whether its own decision was right, using its own confidence as evidence. That circularity is the core problem, and it's worth being precise about what exactly shifts before reaching for solutions.
...
The first term is covariate shift — a different vehicle, fleet attack mix, or wear-induced drift changes which states are seen, but the proxy is still well-calibrated on those states. Standard distribution-shift remedies apply: importance reweighting against $p_{\text{deploy}}/p_{\text{train}}$.
...
- *Bayesian reward as a prior*. Treat the reward population as a posterior $p(R \mid \mathcal{D})$ and act under the posterior mean with a CVaR penalty on the lower tail.
...
- *Thompson sampling over the per-arm reward posterior* [@riquelme2018deep] — a drop-in randomised alternative to UCB with equivalent theoretical coverage.
...
5. Neural-LinUCB on the unprojected logits provides UCB-driven deferral when the *whole policy* is uncertain, regardless of which expert.
