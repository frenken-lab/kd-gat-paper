...
As an example, an umpire calling a pitch has two sources of information: where the ball crossed the plate (physics — trajectory, release point, spin) and what pitches from this pitcher have looked like all game (data — frequency, pattern). If the pitcher throws something he's never thrown before, the umpire's historical read is useless and could even be counterproductive. The call therefore will likely rely from geometry alone. If the ball clips a corner the umpire has been miscalling all night, the physics is right but the confidence is miscalibrated. Neither source is always dominant — the question is which one is _qualified_ on this pitch.
