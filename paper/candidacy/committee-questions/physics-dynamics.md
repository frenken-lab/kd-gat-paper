...
These three failure modes also appear in the CAN bus context which can be more explicitly defined below:
...
- **Model fidelity** ($\mathcal{V}_{\text{residual}}$): The gate fires when the residual is implausible _given the regime_, not just large. A large residual in a hard cornering maneuver is expected; the same residual on a straight highway is not. This operationalizes Q2.1's epistemic/aleatoric decomposition at runtime — regime-conditioned plausibility, not absolute magnitude. In practice, $p(r_t \mid \text{benign}, s_t)$ is estimated empirically: held-out benign data is partitioned into regime buckets (by $v_x$ and $\dot{\psi}$), and $\tau_{\text{ood}}$ is set to the bucket-conditional empirical quantile. The gate checks whether the observed residual falls outside that bucket's typical range.
