...
![Structure of a standard CAN 2.0B data frame. The 29-bit arbitration field determines message priority, while the 0--8 byte data field carries the payload used for feature extraction in our graph construction pipeline.](https://frenken-lab.github.io/kd-gat-paper/assets/images/CANframe.svg){#fig-can-frame width="100%"}
...
where $\sigma(\cdot)$ here denotes the sigmoid function (distinct from the activation in Eq. @eq-gat-update) and $z_i^\top z_j$ measures similarity in latent space. This inner product decoder encourages connected nodes to have similar embeddings.
