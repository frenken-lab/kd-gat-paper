...
::: {.algorithm #alg-graph-construction}
...
::: {.callout-note collapse="false" title="Architectural Enhancements to VGAE"}
...
**Knowledge Distillation:** Student GAT mimics the pre-trained teacher via logit-level distillation (Eq. @eq-kd-total-loss), using temperature-scaled soft targets (Eq. @eq-temperature-scaling) with $T=4$ and mixing coefficient $\lambda=0.7$. No intermediate feature distillation is applied.

::: {.callout-note collapse="false" title="Architectural Enhancements to GAT"}
...
*LSTM Jumping Knowledge.* Layer outputs are aggregated via LSTM-based Jumping Knowledge (Eq. @eq-jk-lstm) rather than concatenation, enabling per-node adaptive depth selection while keeping the output dimension at $d$.
...
The DQN extends the backbone with a linear output layer producing $K$ Q-values, one per discrete fusion weight. Because each CAN window graph is classified independently---the fusion decision for one window does not affect the next---the discount factor is set to $\gamma = 0$. This reduces the Bellman target (Equation @eq-bellman) to pure reward maximization:
